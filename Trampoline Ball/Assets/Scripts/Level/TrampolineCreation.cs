    ...
    public class TrampolineCreation : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            else if (Input.GetMouseButton(0))
            {
                ...
                double angle = Mathf.Atan(pos.x - _startingPosition.x / pos.y - _startingPosition.y) * Mathf.Rad2Deg;
                cube.transform.rotation = Quaternion.Euler(0,0,(float) angle);

                var between = pos - _startingPosition;
                var distance = between.magnitude;
                cube.transform.localScale = new Vector3(cube.transform.localScale.x, cube.transform.localScale.y, distance);
